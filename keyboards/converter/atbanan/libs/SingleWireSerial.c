...
bool _ss_twoWire;
bool _ss_waitBeforeSending asm("_waitBeforeSending") __attribute__ ((used));
bool _ss_buffer_overflow asm("_buffer_overflow") __attribute__ ((used));
uint16_t _ss_bitDelay asm("_bitDelay") __attribute__ ((used)) = 0;
uint16_t _ss_oneAndAHalfBitDelay asm("_oneAndAHalfBitDelay")  __attribute__ ((used));
uint16_t _ss_endOfByte;

uint8_t _ss_setICfalling asm("_setICfalling") __attribute__ ((used));
uint8_t _ss_setICrising asm("_setICrising") __attribute__ ((used));
uint8_t _ss_setCTC asm("_setCTC") __attribute__ ((used));

uint8_t _ss_receive_buffer[_SS_MAX_RX_BUFF] asm("_receive_buffer")  __attribute__ ((used));
volatile uint8_t _ss_receive_buffer_tail asm("_receive_buffer_tail") = 0;
volatile uint8_t _ss_receive_buffer_head asm("_receive_buffer_head") = 0;
...
#if _DEBUG
inline __attribute__ ((always_inline)) void ss_DebugPulse(uint8_t signal)
{
  PORTC |= signal;
  PORTC &= ~signal;
}
#else
inline __attribute__ ((always_inline)) void ss_DebugPulse(__attribute__ ((unused)) uint8_t signal) {}
#endif
...
{
  if (enable) {
    TCCRB = _ss_setICfalling; // look for falling edge of start bit
...
void ss_begin(long speed)
{
  ...
  _ss_waitBeforeSending = true;

  // clear read buffer
   _ss_receive_buffer_tail = _ss_receive_buffer_head;
   _ss_buffer_overflow = false;
  ...
  _ss_setICfalling = _BV(ICNC) | prescaler;
  _ss_setICrising = _ss_setICfalling | _BV(ICES);
  _ss_setCTC = _BV(WGM2) | prescaler;

  _ss_bitDelay = (bit_delay100+50)/100; // bit delay time in timer1 ticks
  _ss_oneAndAHalfBitDelay = (bit_delay100+bit_delay100/2+50)/100; // delay until first sample time point
  _ss_endOfByte = _ss_oneAndAHalfBitDelay + (7*_ss_bitDelay); // last sample timepoint
...
{
  // Empty buffer?
  if (_ss_receive_buffer_head == _ss_receive_buffer_tail)
    return -1;

  // Read from "head"
  uint8_t d = _ss_receive_buffer[_ss_receive_buffer_head]; // grab next byte
  _ss_receive_buffer_head = (_ss_receive_buffer_head + 1) % _SS_MAX_RX_BUFF;
  return d;
}

int ss_available()
{
  return ((unsigned int)(_SS_MAX_RX_BUFF + _ss_receive_buffer_head - _ss_receive_buffer_tail)) % _SS_MAX_RX_BUFF;
}
...
size_t ss_write(uint8_t data)
{
  ...
  if (_ss_waitBeforeSending) { // wait here, because we do not wait to the end of the stop bit when reading
    _ss_waitBeforeSending = false;
    OCRA = _ss_bitDelay << 1; // two bit times
    while (!(TIFR & _BV(OCFA)));
    OCRA = _ss_bitDelay-1;
    TCNT = 0;
    TIFR |= _BV(OCFA);
  ...
  ss_setRxIntMsk(true); //enable input capture input interrupts again
...
int ss_peek()
{
  ...
  // Read from "head"
  return _ss_receive_buffer[_ss_receive_buffer_head];
}

bool ss_overflow()
{
  bool ret = _ss_buffer_overflow;
  if (ret)
    _ss_buffer_overflow = false;
  return ret;
}
